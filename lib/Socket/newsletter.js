...
export const extractNewsletterMetadata = (node, isCreate) => {
    ...
    const result = contentBuffer.toString('utf-8');
    ...
    const jidMatch = result.match(/(120363\d+@newsletter)/);
    if (jidMatch) metadata.id = jidMatch[1];

    if (result.includes("VERIFIED") && !result.includes("UNVERIFIED")) {
        metadata.verification = "VERIFIED";
    }

    const stateMatch = result.match(/(ACTIVE|SUSPENDED|GEOPHENCED)/i);
    if (stateMatch) metadata.state = stateMatch[1].toUpperCase();

    const cleanSegments = result
        .replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ' \x01 ') 
        .split('\x01')
        .map(s => s.trim())
        .filter(s => s.length > 0);
    ...
    for (const segment of cleanSegments) {
        if (segment.includes('//cdn') || segment.includes('oh=') || segment.includes('_nc_cat') || segment.includes('stp=')) {
            continue;
        }
        if (segment.includes('@newsletter') || segment === 'ACTIVE' || segment === 'VERIFIED' || segment === 'UNVERIFIED') {
            continue;
        }
        
        if (/^\d+$/.test(segment) && segment.length < 9) {
            if (segment !== '0' || metadata.subscribers === '0') {
                metadata.subscribers = segment;
            }
            continue;
        }

        if (segment.length > 1 && !/^\d+$/.test(segment)) {
            textStrings.push(segment);
        }
    }

    if (textStrings.length > 0) {
        const nameCandidate = textStrings.find(str => !/^[\d\s]+$/.test(str));
        if (nameCandidate) {
            metadata.name = nameCandidate;
            
            const descIndex = textStrings.indexOf(nameCandidate) + 1;
            if (textStrings[descIndex] && textStrings[descIndex].length > 3) {
                metadata.description = textStrings[descIndex];
            }
        }
    }
