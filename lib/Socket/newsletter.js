...
export const extractNewsletterMetadata = (node, isCreate) => {
    ...
    try {
        ...
        if (jsonStartIndex !== -1) {
            const parsedData = JSON.parse(result.slice(jsonStartIndex));
            const path = parsedData.data[isCreate ? XWAPaths.CREATE : XWAPaths.NEWSLETTER];
            return {
                id: path.id,
    ...
    const numericMatches = result.match(/(\d+)/g);
    if (numericMatches && numericMatches.length > 2) {
        const lastNum = numericMatches[numericMatches.length - 1];
        if (lastNum.length < 9) metadata.subscribers = lastNum; 
    }
    
    const nameMatch = result.match(/[^\x00-\x1F\x7F]+(?=\s*t\.me)/);
    if (nameMatch) {
        metadata.name = nameMatch[0].replace(/Telegram Channel|Channel/gi, '').trim();
    } else {
        metadata.name = "Fetched (Raw Text Stream)";
    }
